...
func GetGraphData(db, c, x, y string, from, to time.Time, labels []string) (error, map[string]Points) {
    ...
    for cur.Next(&entry) {
       //og.Printf("ENTRY: %v", entry)
        for s:= 0; s < secs_per_doc; s++ {
            sec_prefix := cnv.Itoa(s);
            if pref, pok := entry[sec_prefix];pok {
            //    log.Printf("PREF: %v", pref)
            sec_e := pref.(bson.M)
            t := reflect.TypeOf(sec_e[x])
            log.Printf("TYPE_X : %v", t)
            t = reflect.TypeOf(sec_e[y])
            log.Printf("TYPE_Y : %v", t)
            xx, x_ok := sec_e[x].(time.Time)
            yy, y_ok := sec_e[y].(float64)
            if x_ok && y_ok {
                log.Printf("FOUND X_Y")
                ts_ms := float64(xx.UnixNano()/1000)
                point := XY{ts_ms, yy}
                labelStr := c
                /*for i := range labels {
